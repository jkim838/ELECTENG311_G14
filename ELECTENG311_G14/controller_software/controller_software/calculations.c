...
#define ADC_RESOLUTION 1024
	...
	#define CURRENT_RESISTOR_C 0.761
#else
...
double calculate_power(uint8_t raw_coil_voltage, uint8_t raw_coil_current, double PWM_live_time, double cycle_period){
	...
	analog_coil_voltage = (raw_coil_voltage * (ADC_REFERENCE_VOLTAGE / ADC_RESOLUTION)) * (1 + VOLTAGE_SHUNT_A/VOLTAGE_SHUNT_B);	 
	// convert binary coil current (i.e. ADC output) to an analog value...
	analog_coil_current = (raw_coil_current * (ADC_REFERENCE_VOLTAGE / ADC_RESOLUTION) / CURRENT_RESISTOR_C);
